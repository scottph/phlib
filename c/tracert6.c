...
icmp_send (void *v_pszDst)
{
	...
	uiIcmp6Len = sizeof(*pstIcmp6Hdr);
	pszIcmp6 = (char *)calloc(uiIcmp6Len, sizeof(char));

	for (iLoop = 0; iLoop < iMaxTtl; iLoop++)
	{
		icmp_pack ((struct S_icmp6_hdr *) pszIcmp6, g_iSeq);
		sendto (iSock, pszIcmp6, uiIcmp6Len, 0, (struct sockaddr *) &stDst,
				sizeof (stDst));
		g_iSeq++;
		sleep (1);
	}
...
static int
icmp_unpack (void *v_pvFrom, struct S_icmp6_hdr *v_pstIcmp6, int v_iLen)
{
	...
	pstIcmp6Hdr = v_pstIcmp6;
	iIcmpLen = v_iLen;
	iSeq = ntohs(pstIcmp6Hdr->seq);
...
icmp_rcv (void *v_argv)
{
	...
	struct S_icmp6_hdr *pstIcmp6 = NULL;
	...
	while (1)
	{
		iRcvLen = recvmsg (iSock, &stMsg, 0);
                ...
                pstIcmp6 = (struct S_icmp6_hdr *)acRcvBuf;   
		iRet = icmp_unpack (&stFrom, pstIcmp6, iRcvLen);
	}
